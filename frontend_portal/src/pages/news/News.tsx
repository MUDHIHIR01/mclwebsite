...
const ActionButtons: React.FC<ActionButtonsProps> = ({ newsId, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/news/${newsId}`);
      toast.success('News record deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete news record.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/news/${newsId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this news record?</p>
...
const News: React.FC = () => {
  ...
  const fetchNews = useCallback(async () => {
    ...
    try {
      ...
      setData(response.data.news || response.data);
    } catch (err: any) {
      const errorMessage = 'Failed to fetch news records: ' + (err.response?.data?.error || err.message || 'Unknown error');
  ...
  const columns: readonly Column<NewsData>[] = useMemo(
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: CellProps<NewsData>) => {
      ...
      { Header: 'Category', accessor: 'category' },
      {
        ...
        Cell: ({ value }: CellProps<NewsData, string | null>) => <DescriptionCell value={value} />,
      },
      {
        Header: 'Image',
        accessor: 'news_img',
      ...
      },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<NewsData, string>) => new Date(value).toLocaleDateString(),
      },
      {
        Header: 'Actions',
        accessor: 'news_id',
        Cell: ({ row }: CellProps<NewsData>) => (
          <ActionButtons newsId={row.original.news_id} onDeletionSuccess={fetchNews} />
        ),
      },
    ],
    [fetchNews]
          ...
          <table {...getTableProps()} className="w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                ...
                <tr><td colSpan={columns.length} className="text-center py-10 text-gray-500">No news records found matching your criteria.</td></tr>
              )}
            </tbody>
          </table>
        </div>
