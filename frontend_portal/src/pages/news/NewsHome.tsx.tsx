...
const ActionButtons: React.FC<ActionButtonsProps> = ({ news_home_id, onDeletionSuccess }) => {
  ...
  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/news-homes/${news_home_id}`);
      toast.success('News home entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete news home entry.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/news/home/${news_home_id}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this news home entry?</p>
...
export default function NewsHome() {
  ...
  const fetchNewsHomes = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      ...
      toast.error('Failed to fetch news home entries.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('News Home Entries', 20, 10);
    autoTable(doc, {
    ...
    doc.save('news_home_entries.pdf');
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'NewsHomes');
    XLSX.writeFile(workbook, 'news_home_entries.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">News Home Management</h2>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <input
            ...
            placeholder="Search news home entries..."
              ...
              {page.length > 0 ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="text-center py-10 text-gray-500">
                    No news home entries found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
