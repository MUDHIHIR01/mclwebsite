...
const EditGivingBackHome = () => {
  const navigate = useNavigate();
  const { giving_backId } = useParams<{ giving_backId: string }>();
  ...
  useEffect(() => {
    const fetchGivingBackHome = async () => {
      ...
      try {
        const response = await axiosInstance.get<GivingBackHomeData>(`/api/giving-back-homes/${giving_backId}`);
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.put(`/api/giving-back-homes/${giving_backId}`, payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Giving back home entry updated successfully');
      ...
      const errorMessage = error.response?.data?.error || 'Failed to update giving back home entry';
