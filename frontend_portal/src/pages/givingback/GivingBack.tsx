...
const ActionButtons: React.FC<ActionButtonsProps> = ({ givingId, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/giving-back/${givingId}`);
      toast.success('Giving Back record deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      console.error('Delete error:', err);
      toast.error('Failed to delete Giving Back record.', { position: 'top-right' });
    }
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit-giving/back/${givingId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this Giving Back record?</p>
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50" role="dialog" aria-modal="true" onClick={onClose}>
      <div className="relative bg-white rounded-lg p-4 w-full max-w-3xl" onClick={(e) => e.stopPropagation()}>
        ...
        <img
          ...
          onError={(e) => {
            console.warn('Error loading modal image:', imageUrl);
...
export default function GivingBack() {
  ...
  const fetchGivingBack = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get('/api/giving-back');
      setData(response.data.giving_back || response.data);
    } catch (err: any) {
      console.error('Fetch error:', err);
      const errorMessage = err.response?.data?.error || 'Failed to fetch Giving Back records.';
          ...
          return (
            <div className="flex gap-2">
              {images.map((image, index) => {
                ...
                return (
                  <button key={index} onClick={() => setSelectedImage(imageUrl)} className="focus:outline-none" aria-label={`View image ${index + 1}`}>
                    <img
                      src={imageUrl}
                      alt={`Giving Back Image ${index + 1}`}
                      className="h-16 w-16 object-cover rounded cursor-pointer hover:opacity-80 transition"
                      onError={(e) => {
                        console.warn('Error loading thumbnail image:', imageUrl);
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Giving Back Management</h2>
              ...
              {page.length > 0 ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="text-center py-10 text-gray-500">
                    No Giving Back records found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
            ...
            >
              {[5, 10, 20, 30, 50].map((size) => (
                <option key={size} value={size}>Show {size} rows</option>
              ))}
            </select>
          </div>
        )}
