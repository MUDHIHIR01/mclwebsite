...
const ActionButtons: React.FC<ActionButtonsProps> = ({ serviceId, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/services/${serviceId}`);
      toast.success('Service record deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete service record.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/services/edit/${serviceId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this service record?</p>
...
export default function Services() {
    ...
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: CellProps<ServiceData>) => {
      ...
      { Header: 'Service Category', accessor: 'service_category' },
      {
        ...
        Cell: ({ value }: CellProps<ServiceData, string | null>) => <DescriptionCell value={value} />,
      },
      {
        Header: 'Image',
      ...
      },
      {
        Header: 'URL Link',
        accessor: 'url_link',
        Cell: ({ value }: CellProps<ServiceData, string | null>) => <UrlLinkCell value={value} />,
      },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<ServiceData, string>) => new Date(value).toLocaleDateString(),
      },
      {
        Header: 'Actions',
        accessor: 'service_id',
        Cell: ({ row }: CellProps<ServiceData>) => (
          <ActionButtons serviceId={row.original.service_id} onDeletionSuccess={fetchServices} />
        ),
      },
    ],
    [fetchServices]
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      head: [['#', 'Service Category', 'Description', 'URL Link', 'Created At']],
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Service Management</h2>
          <Link to="/services/add" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
          ...
          <table {...getTableProps()} className="w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                ...
                <tr><td colSpan={columns.length} className="text-center py-10 text-gray-500">No service records found matching your criteria.</td></tr>
              )}
            </tbody>
          </table>
        </div>
