...
import {
  ...
  WrenchScrewdriverIcon, // Icon for Services
...
// --- REFINED: Services Home Slideshow ---
  ...
  if (error || data.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[80vh] bg-gradient-to-br from-indigo-600 to-purple-700 p-6">
        ...
        {error && <button onClick={fetchServicesHome} className="inline-flex items-center px-8 py-3 text-white rounded-full hover:brightness-90 transition-all shadow-lg" style={{ backgroundColor: '#d12814' }}><ArrowPathIcon className="w-5 h-5 mr-2" /> Try Again</button>}
      </div>
    );
  }
...
// --- REFINED: Individual Service Card Component ---
    ...
    return (
        ...
        >
                ...
                <div className="h-48 w-full">
                    {showPlaceholder ? (
                        <div className="h-full w-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
                            <WrenchScrewdriverIcon className="w-16 h-16 text-gray-300 dark:text-gray-500" />
                        </div>
                    ) : (
                        <img className="h-full w-full object-cover" src={imageUrl!} alt={service.service_category} onError={() => setHasImageError(true)} />
                    )}
                </div>
            ...
            <div className="p-6 flex flex-col flex-grow">
                <h3 className="text-lg font-bold" style={{ color: '#d12814' }}>{service.service_category}</h3>
                <p className="mt-2 text-gray-600 dark:text-gray-300 text-sm flex-grow font-semibold">{service.description}</p>
                <div className="mt-6">
                    {service.url_link && (
                        <a
...
// --- REFINED: Services Section Component ---
    ...
    <section className="bg-gray-50 dark:bg-gray-900 py-16 sm:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
            <h2 className="text-base font-semibold text-[#0069b4] dark:text-indigo-400 tracking-wide uppercase">What We Offer</h2>
