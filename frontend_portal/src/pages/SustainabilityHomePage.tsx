  ...
  if (error || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] text-center p-6 bg-gray-800">
        ...
        {error && (
          <button onClick={fetchSustainabilityHomes} className="mt-6 flex items-center px-6 py-3 bg-gray-800 text-white font-semibold rounded-full hover:bg-gray-700 transition">
  ...
  if (error || sustainabilityData.length === 0) {
    return (
      <div className="w-full py-20 flex flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mt-2 text-gray-600">{error || "There is no sustainability data to display at the moment."}</p>
        {error && (
          <button onClick={fetchSustainabilityData} className="mt-6 flex items-center px-6 py-3 bg-gray-800 text-white font-semibold rounded-full hover:bg-gray-700 transition">
            <ArrowPathIcon className="w-5 h-5 mr-2" />Retry
          </button>
        )}
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mt-12">
          {sustainabilityData.map((item) => (
            <SustainabilityCard key={item.sustain_id} item={item} />
          ))}
        </div>
      </div>
    </section>
