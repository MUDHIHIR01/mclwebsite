...
const EditLeadershipHome = () => {
  ...
  useEffect(() => {
    const fetchLeadershipHome = async () => {
      ...
      try {
        const response = await axiosInstance.get<LeadershipHomeData>(`/api/leadership-homes/${leadership_home_id}`);
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.put(`/api/leadership-homes/${leadership_home_id}`, payload, {
      ...
      const errorMessage = error.response?.data?.error || 'Failed to update leadership home entry';
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={() => navigate('/leadership/home')}
