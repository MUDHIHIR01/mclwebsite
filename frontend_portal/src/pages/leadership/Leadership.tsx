...
const ActionButtons: React.FC<ActionButtonsProps> = ({ leadershipId, onDeletionSuccess }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success('Leadership record deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit-leadership/${leadershipId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
...
export default function Leadership() {
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      head: [['#', 'Leader Name', 'Position', 'Description', 'Created At']],
