        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <div className="flex justify-end gap-4">
              <button onClick={() => setShowConfirm(false)} className="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400">Cancel</button>
...
// Reusable DescriptionCell component
...
export default function Diversity() {
  ...
  const columns: readonly Column<DiversityData>[] = useMemo(
    () => [
      { Header: '#', id: 'rowIndex', Cell: ({ row, flatRows }: CellProps<DiversityData>) => <span>{flatRows.findIndex(fr => fr.id === row.id) + 1}</span> },
      { Header: 'Category', accessor: 'diversity_category' },
      { Header: 'Description', accessor: 'description', Cell: ({ value }: { value: string }) => <DescriptionCell value={value} /> },
      {
        Header: 'PDF File',
        accessor: 'pdf_file',
        Cell: ({ value }: CellProps<DiversityData, string | null>) => {
          ...
          const pdfUrl = `${baseUrl.replace(/\/$/, '')}/storage/${value}`;
          return <a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">View PDF</a>;
        },
      },
      { Header: 'Created At', accessor: 'created_at', Cell: ({ value }: { value: string }) => new Date(value).toLocaleDateString() },
      { Header: 'Actions', accessor: 'diversity_id', Cell: ({ row }: CellProps<DiversityData>) => <ActionButtons diversityId={row.original.diversity_id} onDeletionSuccess={fetchDiversityRecords} /> },
    ],
    [fetchDiversityRecords]
  );
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      ...
      body: data.map((row, i) => [i + 1, row.diversity_category, row.description, new Date(row.created_at).toLocaleDateString()]),
    });
  ...
  const exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(data.map((row, i) => ({ '#': i + 1, Category: row.diversity_category, Description: row.description, 'Created At': new Date(row.created_at).toLocaleDateString() })));
  ...
  if (error && data.length === 0) return <div className="text-center p-10 text-red-500">{error} <button onClick={fetchDiversityRecords} className="ml-2 text-blue-500">Try Again</button></div>;
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          ...
          <Link to="/add/diversityInclusion" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <input value={globalFilter || ''} onChange={(e) => setGlobalFilter(e.target.value)} placeholder="Search records..." className="px-4 py-2 border border-gray-300 rounded-lg w-full sm:w-64" />
        ...
        <div className="overflow-x-auto">
          <table {...getTableProps()} className="w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              {headerGroups.map(hg => (<tr {...hg.getHeaderGroupProps()}>{hg.headers.map(col => (<th {...col.getHeaderProps()} className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{col.render('Header')}</th>))}</tr>))}
            </thead>
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                page.map(row => { prepareRow(row); return (<tr {...row.getRowProps()} className="hover:bg-gray-50">{row.cells.map(cell => (<td {...cell.getCellProps()} className="px-4 py-4 text-sm">{cell.render('Cell')}</td>))}</tr>); })
              ) : (
