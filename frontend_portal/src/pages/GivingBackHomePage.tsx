...
import {
  ...
  HeartIcon, // Icon for Giving Back
...
interface GivingBackData {
  ...
  description: string;
  weblink: string | null;
  image_slider: string; // JSON string of image paths
}

// --- REFINED: Giving Back Home Slideshow ---
  ...
  if (error || data.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[80vh] bg-gradient-to-br from-indigo-600 to-purple-700 p-6">
        ...
        {error && <button onClick={fetchGivingBackHomes} className="inline-flex items-center px-8 py-3 text-white rounded-full hover:brightness-90 transition-all shadow-lg" style={{ backgroundColor: '#d12814' }}><ArrowPathIcon className="w-5 h-5 mr-2" /> Try Again</button>}
      </div>
    );
  }
...
const GivingBackCard: React.FC<{ item: GivingBackData }> = ({ item }) => {
    ...
    // Safely parse the image slider JSON and get the first image
    const getFirstImage = () => {
        try {
    ...
    return (
        ...
        >
                ...
                <div className="h-48 w-full">
                    {showPlaceholder ? (
                        <div className="h-full w-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
                            <HeartIcon className="w-16 h-16 text-gray-300 dark:text-gray-500" />
                        </div>
                    ) : (
                        <img className="h-full w-full object-cover" src={imageUrl!} alt={item.givingBack_category} onError={() => setHasImageError(true)} />
                    )}
                </div>
            ...
            <div className="p-6 flex flex-col flex-grow">
                <h3 className="text-lg font-bold" style={{ color: '#d12814' }}>{item.givingBack_category}</h3>
...
// --- REFINED: Giving Back Section Component ---
const GivingBackSection: React.FC = () => {
  ...
  const fetchGivingBackData = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Error fetching Giving Back data.");
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <section className="bg-gray-50 dark:bg-gray-900 py-16 sm:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
            <h2 className="text-base font-semibold text-[#0069b4] dark:text-indigo-400 tracking-wide uppercase">Community Commitment</h2>
        ...
        {loading ? (
          ...
          <div className="mt-12 grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="text-center py-10 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-xl">No "Giving Back" initiatives found.</p>
            </div>
        )}
      </div>
    </section>
