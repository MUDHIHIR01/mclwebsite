...
import {
  ...
  SparklesIcon, // A more suitable icon for benefits
...
// --- REFINED: Benefits Home Slideshow ---
const BenefitsHomeSlideshow: React.FC = () => {
  ...
  const fetchBenefitHomes = useCallback(async () => {
    ...
    setError(null);
    try {
      // Fetching from the new benefits slider endpoint
  ...
  if (error || data.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[80vh] bg-gradient-to-br from-indigo-600 to-purple-700 p-6">
        ...
        <p className="text-gray-200 mb-8 text-lg text-center">{error || "Content for the benefits slider could not be loaded."}</p>
        {error && <button onClick={fetchBenefitHomes} className="inline-flex items-center px-8 py-3 text-white rounded-full transition-all duration-300 shadow-lg hover:brightness-90" style={{ backgroundColor: '#d12814' }}><ArrowPathIcon className="w-5 h-5 mr-2" />Try Again</button>}
      </div>
    );
  }
...
// --- REFINED: Individual Benefit Card ---
const BenefitCard: React.FC<{ benefit: Benefit }> = ({ benefit }) => {
    const [hasImageError, setHasImageError] = useState(false);
    const imageUrl = `${axiosInstance.defaults.baseURL?.replace(/\/$/, "")}/${benefit.img_file.replace(/^\//, "")}`;
    ...
    return (
        ...
        >
            <div className="relative">
                <div className="h-48 w-full">
                    {hasImageError ? (
                        ...
                        <img className="h-full w-full object-cover" src={imageUrl} alt={benefit.category} onError={() => setHasImageError(true)} />
                    )}
                </div>
            ...
            <div className="p-6 flex flex-col flex-grow">
                <h3 className="text-lg font-bold" style={{ color: '#d12814' }}>{benefit.category}</h3>
                <p className="mt-2 text-gray-600 dark:text-gray-300 text-sm flex-grow font-semibold">{benefit.description}</p>
                {/* "View More" button is removed as the description is already shown on the card */}
            </div>
        </motion.div>
    );
};
...
const BenefitsSection: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      // Fetching from the new benefits list endpoint
      const response = await axiosInstance.get<{ benefits: Benefit[] }>("/api/benefits/all");
      if (response.data && Array.isArray(response.data.benefits)) {
        setBenefits(response.data.benefits);
      } else {
        toast.error("Failed to fetch benefits: Invalid data format.");
        setBenefits([]);
      }
    } catch (err) {
      toast.error("An error occurred while fetching benefits.");
      setBenefits([]);
    } finally {
      setIsLoading(false);
    }
    ...
    <section className="bg-gray-50 dark:bg-gray-900 py-16 sm:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
            <h2 className="text-base font-semibold text-[#0069b4] dark:text-indigo-400 tracking-wide uppercase">Our Perks</h2>
          ...
          <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {benefits.map((benefit) => (
             ...
             <div className="text-center py-10 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-xl">No benefits information found at this time.</p>
            </div>
        )}
      </div>
    </section>
